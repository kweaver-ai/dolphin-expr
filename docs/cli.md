# 命令行使用（run / create / analyst）

统一的实验命令入口，覆盖“运行/管理/分析/创建”。

## 命令一览

- `bin/run`: 运行/管理实验（支持恢复、状态、环境枚举、verbose）
- `bin/analyst`: 分析实验（总体分析、单题执行分析、Summary 分析）
- `bin/create`: 从现有 `.dph` 文件夹创建新的实验设计
...
每个执行环境位于 `env/{name}_{timestamp}/`，每次采样生成 `run_XXX/` 目录，包含：
...
- **总体分析**：
  ...
  - `env/{experiment}/reports/{experiment}_general_report_{timestamp}.csv`（包含整体正确率列）

- **执行分析**：
  - 结果输出到控制台（标记为 `===ANALYSIS_START=== ... ===ANALYSIS_END===`）
  - 自动保存到：`env/{experiment}/{run}/analysis/case_XXX.txt`
  - 支持缓存，再次分析同一 case 会使用缓存结果

- **Summary 分析**：
  - 写入对应 run：`env/{experiment}/{run}/summary_result.txt`
  - 基于已保存的 analysis 结果进行汇总
  - 支持业务知识增强，提供更精准的改进建议

- **跨run分析**：
  - 分析结果保存到各个 run 的 analysis 目录
  - 汇总报告：`env/{experiment}/analysis/cross_run_summary_{timestamp}.txt`
  - 包含跨run的高频错误分析、遗漏业务知识识别和改进建议

### 知识路径查找规则（--knows）
- 相对路径时的搜索顺序：
  1) 单 run 的 Summary/执行分析：优先 `{env}/{run}/<knows>`
  2) 设计目录：`design/{design_name}/<knows>`（如 watsons_baseline_20250914_XXXX -> 设计名 watsons_baseline）
  3) 实验环境根目录：`{env}/<knows>`
  4) 项目根目录、当前工作目录
- 绝对路径：直接使用

更多分析维度与能力详见 `docs/analyst.md`。
...
- `design/my_experiment/dolphins/`（复制源 `.dph`）
...
- 结果分析：
  ...
  - `./bin/analyst demo_20250901_120000 --analysis --run run_001`  # 批量分析
  - `./bin/analyst demo_20250901_120000 --analysis --run run_001 --knows ./docs/`  # 使用知识
  - `./bin/analyst demo_20250901_120000 --analysis --run run_001 --summary --knows ./docs/`  # Summary+知识
