...
这个工具自动分析 `env` 下的实验结果，生成结构化的文本报告与详细 CSV 数据。
...
### 分析报告
分析完成后，会在 `env/{实验环境ID}/reports/` 目录下生成两个文件：
...
实验路径: /full/path/to/env/bird_baseline_20250809_155347
...
dolphin-expr/
├── analyst/                   # 分析工具目录
│   ├── experiment_analyzer.py  # 数据加载器/兼容入口
│   └── README.md              # 说明文档
└── env/                       # 实验结果目录
    └── bird_baseline_20250804_105810/  # 具体实验
        └── reports/            # 分析报告输出目录
            ├── *_analysis_*.txt
            └── *_analysis_*_detailed.csv
```
...
```bash
# 分析单个case
./bin/analyst my_experiment --analysis --run run_001 --case 001

# 批量分析失败的cases
./bin/analyst my_experiment --analysis --run run_001

# 使用业务知识增强分析
./bin/analyst my_experiment --analysis --run run_001 --case 001 --knows business_rules.txt
```
...
```bash
# 分析正确率低于30%的cases
./bin/analyst my_experiment --cross-run-analysis --max-accuracy 30

# 跨run分析并生成汇总报告
./bin/analyst my_experiment --cross-run-analysis --max-accuracy 30 --summary

# 针对单个case的跨run分析
./bin/analyst my_experiment --cross-run-analysis --case 001 --summary
```
...
```bash
# 处理指定的case
./bin/analyst my_experiment --sim-inject --case_id 001
...
./bin/analyst my_experiment --sim-inject

# 处理所有低于30%准确率的cases
./bin/analyst my_experiment --sim-inject --accuracy-threshold 30
...
2. **渐进式优化（的先使用语义驱动模式）**:
   ...
   # 2. 语义驱动批量测试
   ./bin/analyst my_experiment --sim-inject --accuracy-threshold 10

   # 3. 必要时使用传统模式进行对比
   ./bin/analyst my_experiment --sim-inject --accuracy-threshold 20 --max-iterations 10
   ```
...
```bash
# 生成run级别的汇总分析
./bin/analyst my_experiment --analysis --run run_001 --summary
```
...
1. **语义裁判失败**
   ...
   ls analyst/dolphins/semantic_judge.dph
...
2. **优化不收敛或效果差**
   ```bash
   # 降低学习率和增加耐心值
   ./bin/analyst my_exp --sim-inject --case_id 001 \
